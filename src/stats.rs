...
impl Stats {
    pub fn new(fs: &str, mount: &str, statvfs: Statvfs, args: &ArgMatches) -> Stats {
        let size_disk = statvfs.blocks() as u64 * statvfs.fragment_size();
        let fragment_size = statvfs.fragment_size();
        let blocks = statvfs.blocks();
        let available_disk = statvfs.blocks_available() as u64 * statvfs.fragment_size();
        let free_disk = statvfs.blocks_free() as u64 * statvfs.fragment_size();
